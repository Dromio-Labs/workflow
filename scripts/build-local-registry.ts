...
for (const directory of packages) {
  ...
  run("bun", ["pm", "pack", "--destination", packageDir, "--ignore-scripts"], stage);
}

for (const tarballFile of (await readdir(packageDir)).filter((entry) => entry.endsWith(".tgz")).sort()) {
  const tarballPath = path.join(packageDir, tarballFile);
  const bytes = await readFile(tarballPath);
  const packageJson = JSON.parse(capture("tar", ["-xOf", tarballPath, "package/package.json"], root)) as PackageManifest;
  registry.push({
...
await writeFile(path.join(outDir, "package-registry-manifest.json"), `${JSON.stringify(registry, null, 2)}\n`);
...
function rewriteWorkspaceVersions(manifest: PackageManifest, versions: Map<string, PackageManifest>): void {
  for (const field of ["dependencies", "devDependencies", "peerDependencies"] as const) {
    for (const [name, value] of Object.entries(manifest[field] ?? {})) {
      ...
      if (!dependency) throw new Error(`${manifest.name} references unknown workspace package ${name}`);
